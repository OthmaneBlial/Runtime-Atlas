...
import type { AtlasEdge, AtlasNode, AtlasTopology, NodeKind } from "../shared/types.js";
...
function readStringProperty(object: ObjectLiteralExpression, name: string): string | undefined {
...
function readMeta(object: ObjectLiteralExpression): Record<string, string> | undefined {
  ...
  for (const child of initializer.getProperties()) {
    ...
    if (!value || (!Node.isStringLiteral(value) && !Node.isNoSubstitutionTemplateLiteral(value))) continue;
...
export function analyzeProject(entryPatterns: string[]): AtlasTopology {
  ...
  if (!sourceFiles.length) throw new Error(`No source files matched: ${entryPatterns.join(", ")}`);
  const nodes: AtlasNode[] = [];
  type DeclarationRecord = { node: AtlasNode; call: CallExpression; file: string };
  ...
  for (const sourceFile of sourceFiles) {
    for (const declaration of sourceFile.getVariableDeclarations()) {
      ...
      declarationsBySymbol.set(symbol, [...(declarationsBySymbol.get(symbol) ?? []), record]);
    }
  }
  ...
  for (const { node, call, file } of declarations) {
    ...
    for (const nestedCall of nestedCalls) {
      ...
      let target = declarationsByLocalName.get(localKey(file, calledName))?.node;

      if (!target) {
        const sourceFile = call.getSourceFile();
        for (const importDeclaration of sourceFile.getImportDeclarations()) {
          const imported = importDeclaration.getNamedImports().find((namedImport) =>
            (namedImport.getAliasNode()?.getText() ?? namedImport.getName()) === calledName,
          );
          if (!imported) continue;
          const importedFile = importDeclaration.getModuleSpecifierSourceFile()?.getFilePath();
          if (!importedFile) continue;
          target = declarationsByLocalName.get(localKey(importedFile, imported.getName()))?.node;
          if (target) break;
        }
      }
  ...
  return {
    ...
    sourceRoot: entryPatterns.length === 1
      ? path.dirname(path.relative(process.cwd(), sourceFiles[0].getFilePath()))
      : ".",
    nodes,
