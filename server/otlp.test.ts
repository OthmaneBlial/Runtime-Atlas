...
const topology: AtlasTopology = {
  ...
  edges: [
    {
      id: "route.orders->service.checkout",
      source: "route.orders",
    ...
    resourceSpans: [
      {
        resource: {
          attributes: [
            attribute("service.name", { stringValue: "orders-api" }),
          ],
        },
        scopeSpans: [
          {
...
describe("OTLP/HTTP JSON conversion", () => {
  it("reconciles static nodes and infers live dependencies without retaining URL queries", () => {
      ...
      envelope([
        ...
        },
        {
          ...
          attributes: [
            attribute("code.function.name", {
              stringValue: "shop.handlers.checkout",
            }),
            attribute("code.file.path", {
              stringValue: "/workspace/src/checkout.ts",
            }),
        ...
        },
        {
          ...
          attributes: [
            ...
            attribute("url.full", {
              stringValue:
                "https://payments.example/charge?secret=do-not-render",
            }),
          ],
        },
      ]),
    ...
    const starts = converted.events.filter(
      (event) => event.type === "span:start",
    );
    expect(
      starts.find((event) => event.spanId === "1111111111111111")?.nodeId,
    ).toBe("route.orders");
    expect(
      starts.find((event) => event.spanId === "2222222222222222")?.nodeId,
    ).toBe("service.checkout");
    expect(
      starts.find((event) => event.spanId === "3333333333333333"),
    ).toMatchObject({
    ...
    const external = starts.find(
      (event) => event.spanId === "4444444444444444",
    );
  ...
  it("applies HTTP error semantics while respecting explicit OK status", () => {
    ...
    const clientFinish = converted.events.find(
      (event) =>
        event.spanId === "7777777777777777" && event.type !== "span:start",
    );
    const serverFinish = converted.events.find(
      (event) =>
        event.spanId === "8888888888888888" && event.type !== "span:start",
    );
    expect(clientFinish).toMatchObject({
      type: "span:error",
      error: "HTTP 404",
    });
  ...
  it("rejects duplicate span identifiers within a trace", () => {
    ...
    const converted = convertOtlpTraceRequest(
      envelope([duplicate, { ...duplicate, name: "again" }]),
      topology,
    );
  ...
  it("decodes every supported AnyValue shape and enforces the span capacity bound", () => {
    ...
    expect(decodeOtlpAnyValue({ intValue: "9007199254740993" })).toBe(
      "9007199254740993",
    );
    expect(decodeOtlpAnyValue({ doubleValue: 3.14 })).toBe(3.14);
    expect(decodeOtlpAnyValue({ bytesValue: "AQI=" })).toEqual({
      bytesValue: "AQI=",
    });
    expect(
      decodeOtlpAnyValue({
    ...
    expect(
      decodeOtlpAnyValue({
        kvlistValue: {
          values: [{ key: "nested", value: { boolValue: false } }],
        },
      }),
    ).toEqual({ nested: false });
    expect(
      decodeOtlpAnyValue({ stringValue: "ambiguous", boolValue: true }),
    ).toBeUndefined();
    expect(() =>
      convertOtlpTraceRequest(envelope([{}, {}]), topology, 1),
    ).toThrowError(OtlpRequestError);
    try {
      convertOtlpTraceRequest(envelope([{}, {}]), topology, 1);
    } catch (error) {
