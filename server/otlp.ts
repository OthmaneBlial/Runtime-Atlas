...
function boundedString(value: unknown, limit = MAX_STRING_LENGTH): string | undefined {
  if (typeof value !== "string") return undefined;
  return value.slice(0, limit);
}

function readArray(record: JsonRecord, key: string, location: string): unknown[] {
  const value = record[key];
  if (value == null) return [];
  if (!Array.isArray(value)) throw new OtlpRequestError(`${location}.${key} must be an array`);
  return value;
}

/** Decode the protobuf-JSON AnyValue union without coercing unknown fields. */
export function decodeOtlpAnyValue(value: unknown, depth = 0): OtlpAttributeValue | undefined {
  ...
  if (typeof value.stringValue === "string") return value.stringValue.slice(0, MAX_STRING_LENGTH);
  if (typeof value.boolValue === "boolean") return value.boolValue;
  if (typeof value.intValue === "number" && Number.isInteger(value.intValue)) return value.intValue;
  ...
  if (typeof value.bytesValue === "string" && /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(value.bytesValue)) {
  ...
  if (isRecord(value.arrayValue) && Array.isArray(value.arrayValue.values)) {
    return value.arrayValue.values.slice(0, MAX_NESTED_VALUES).map((entry) =>
      decodeOtlpAnyValue(entry, depth + 1) ?? null,
    );
  }
  if (isRecord(value.kvlistValue) && Array.isArray(value.kvlistValue.values)) {
    ...
    for (const item of value.kvlistValue.values.slice(0, MAX_NESTED_VALUES)) {
      ...
      if (decoded !== undefined && !entries.some(([existing]) => existing === key)) {
...
function readAttributes(container: JsonRecord): Map<string, OtlpAttributeValue> {
...
function stringAttribute(attributes: Map<string, OtlpAttributeValue>, key: string): string | undefined {
  const value = attributes.get(key);
  return typeof value === "string" ? value : undefined;
}

function numberAttribute(attributes: Map<string, OtlpAttributeValue>, key: string): number | undefined {
  const value = attributes.get(key);
...
function parseId(value: unknown, length: 16 | 32, optional = false): string | undefined {
  if ((value == null || value === "") && optional) return undefined;
  if (typeof value !== "string" || !new RegExp(`^[0-9a-fA-F]{${length}}$`).test(value)) return undefined;
...
function parseSpan(
  ...
  if (value.parentSpanId != null && value.parentSpanId !== "" && !parentSpanId) {
  ...
  if (start == null || end == null) return "span has an invalid nanosecond timestamp";
  ...
  if (statusCode == null) return "span.status.code must be an integer enum value";
  const serviceName = stringAttribute(resourceAttributes, "service.name") || "unknown_service";
  const serviceNamespace = stringAttribute(resourceAttributes, "service.namespace");
...
function inferredKind(span: ParsedSpan): NodeKind {
  const override = stringAttribute(span.attributes, "runtime.atlas.kind") as NodeKind | undefined;
  if (override && NODE_KINDS.has(override)) return override;
  const databaseSystem = stringAttribute(span.attributes, "db.system.name")?.toLowerCase();
  if (databaseSystem) return CACHE_SYSTEMS.has(databaseSystem) ? "cache" : "database";
  if (stringAttribute(span.attributes, "messaging.system")) return "queue";
  if (span.kind === 2 && stringAttribute(span.attributes, "http.request.method")) return "route";
  if (span.kind === 3 && (
...
function nodePresentation(span: ParsedSpan, kind: NodeKind): Omit<ResolvedNode, "nodeId" | "kind"> & { key: string } {
  const method = stringAttribute(span.attributes, "http.request.method")?.toUpperCase();
  ...
  const destination = stringAttribute(span.attributes, "messaging.destination.name");
  ...
  switch (kind) {
    ...
    case "database":
    case "cache": {
      const system = databaseSystem ?? (kind === "cache" ? "Cache" : "Database");
    ...
    case "queue": {
      const label = [messagingSystem ?? "Message broker", destination].filter(Boolean).join(" / ");
      return {
...
function resolveStaticNode(span: ParsedSpan, topology: AtlasTopology): AtlasNode | undefined {
  ...
  const codeFile = stringAttribute(span.attributes, "code.file.path")
    ?? stringAttribute(span.attributes, "code.filepath");
  const codeLine = numberAttribute(span.attributes, "code.line.number")
  ...
  if (codeFile && codeLine != null) {
    ...
    const sourceMatches = topology.nodes.filter((node) => {
      ...
      return node.source.line === codeLine && (sought === candidate || sought.endsWith(`/${candidate}`));
    });
  ...
  if (inferredKind(span) === "route") {
    ...
    const route = stringAttribute(span.attributes, "http.route");
    if (method && route) {
      const routeMatches = topology.nodes.filter((node) =>
        node.kind === "route" &&
...
function resolveNode(span: ParsedSpan, topology: AtlasTopology): ResolvedNode {
  ...
  if (staticNode) {
    return {
      ...
      description: staticNode.description ?? `Static node reconciled with ${span.name}.`,
    };
  }
  ...
  const explicitId = explicitNodeId(span);
  return {
    nodeId: explicitId?.slice(0, 256) || `otlp.${kind}.${slug(span.service)}.${slug(presentation.key)}`,
    kind,
...
function errorForSpan(span: ParsedSpan): string | undefined {
  ...
  if (span.statusCode === 2) return span.statusMessage || errorType || "OpenTelemetry span reported an error";
  if (errorType) return errorType;
  const httpStatus = numberAttribute(span.attributes, "http.response.status_code");
  if (httpStatus != null && (httpStatus >= 500 || (httpStatus >= 400 && span.kind === 3))) {
...
function detailForSpan(span: ParsedSpan, node: ResolvedNode): Record<string, string | number | boolean> {
  ...
  if (span.scopeName) detail["otel.scope"] = span.scopeVersion
    ? `${span.scopeName}@${span.scopeVersion}`
    : span.scopeName;
  if (span.schemaUrl) detail["otel.schema"] = span.schemaUrl;
  const serviceNamespace = stringAttribute(span.resourceAttributes, "service.namespace");
...
function depthOf(span: ParsedSpan, spans: Map<string, ParsedSpan>, visiting = new Set<string>()): number {
...
function requestForTrace(spans: ParsedSpan[]): { request: RuntimeRequest; root: ParsedSpan } {
  const root = spans.find((span) =>
    span.kind === 2 && !span.parentSpanId && stringAttribute(span.attributes, "http.request.method"),
  ) ?? spans.find((span) => span.kind === 2 && stringAttribute(span.attributes, "http.request.method"))
    ?? [...spans].sort((a, b) => a.start < b.start ? -1 : a.start > b.start ? 1 : 0)[0];
  const method = stringAttribute(root.attributes, "http.request.method")?.toUpperCase() ?? "TRACE";
...
function convertTrace(spans: ParsedSpan[], topology: AtlasTopology): RuntimeEventInput[] {
  const byId = new Map(spans.map((span) => [span.spanId, span]));
  const start = spans.reduce((minimum, span) => span.start < minimum ? span.start : minimum, spans[0].start);
  const end = spans.reduce((maximum, span) => span.end > maximum ? span.end : maximum, spans[0].end);
  const { request, root } = requestForTrace(spans);
  const traceError = errorForSpan(root) ?? spans.map(errorForSpan).find(Boolean);
...
export function convertOtlpTraceRequest(
  ...
  if (!isRecord(payload)) throw new OtlpRequestError("OTLP request body must be a JSON object");
  const resourceSpans = readArray(payload, "resourceSpans", "request");
  if (resourceSpans.length > maxSpans) {
    throw new OtlpRequestError(`OTLP request exceeds the ${maxSpans} resource group limit`, 413);
  }
  ...
  for (const [resourceIndex, resourceValue] of resourceSpans.entries()) {
    if (!isRecord(resourceValue)) throw new OtlpRequestError(`resourceSpans[${resourceIndex}] must be an object`);
    const resource = resourceValue.resource == null ? {} : resourceValue.resource;
    if (!isRecord(resource)) throw new OtlpRequestError(`resourceSpans[${resourceIndex}].resource must be an object`);
    const resourceAttributes = readAttributes(resource);
    const resourceSchema = boundedString(resourceValue.schemaUrl, 512);
    const scopeSpans = readArray(resourceValue, "scopeSpans", `resourceSpans[${resourceIndex}]`);
    seenScopes += scopeSpans.length;
    if (seenScopes > maxSpans) {
      throw new OtlpRequestError(`OTLP request exceeds the ${maxSpans} scope group limit`, 413);
    }

    for (const [scopeIndex, scopeValue] of scopeSpans.entries()) {
      if (!isRecord(scopeValue)) {
        throw new OtlpRequestError(`resourceSpans[${resourceIndex}].scopeSpans[${scopeIndex}] must be an object`);
      }
      const scope = scopeValue.scope == null ? {} : scopeValue.scope;
      if (!isRecord(scope)) {
        throw new OtlpRequestError(`resourceSpans[${resourceIndex}].scopeSpans[${scopeIndex}].scope must be an object`);
      }
      ...
      if (seenSpans > maxSpans) {
        throw new OtlpRequestError(`OTLP request exceeds the ${maxSpans} span limit`, 413);
      }
      const schemaUrl = boundedString(scopeValue.schemaUrl, 512) ?? resourceSchema;
  ...
  for (const span of parsed) traces.set(span.traceId, [...(traces.get(span.traceId) ?? []), span]);
  const events = [...traces.values()].flatMap((spans) => convertTrace(spans, topology));
  ...
  return {
    ...
    ...(rejectedSpans ? {
      errorMessage: `${rejectedSpans} span${rejectedSpans === 1 ? " was" : "s were"} rejected: ${[
        ...new Set(rejectionReasons),
      ].slice(0, 3).join("; ")}`.slice(0, 1_024),
    } : {}),
  };
}
