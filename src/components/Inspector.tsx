...
export function Inspector({
    ...
    return (
      <aside className="inspector inspector-empty" aria-label="Node inspector">
        ...
        <div className="inspector-prompt">
          ...
          <p>
            See the source declaration, ownership metadata, and measured
            latency.
          </p>
        </div>
      </aside>
  ...
  const current = visibleEvents
    ...
    .at(-1);
  const average = metric.calls
    ? Math.round(metric.totalDuration / metric.calls)
    : undefined;
    ...
    <aside className="inspector" aria-label={`${node.label} inspector`}>
      ...
      <div className={`inspector-hero kind-${node.kind}`}>
        <span className="inspector-icon">
          <KindIcon kind={node.kind} size={19} />
        </span>
        <small>
          {node.kind} / {node.id}
        </small>
      ...
      <div className="inspector-stats">
        <div>
          <small>LAST</small>
          <strong>
            {metric.lastDuration != null ? `${metric.lastDuration}ms` : "—"}
          </strong>
        </div>
        <div>
          <small>MEAN</small>
        ...
        <div>
          <small>CALLS</small>
          <strong>{metric.calls}</strong>
        </div>
      </div>
      <div className="inspector-section">
        <h3>
          <Braces size={13} /> SOURCE DECLARATION
        </h3>
        {node.source.runtimeOnly ? (
          ...
          <button
            ...
            aria-label={
              copied ? "Source location copied" : "Copy source location"
            }
            onClick={() => {
              if (!navigator.clipboard) return;
              void navigator.clipboard
                .writeText(`${node.source.file}:${node.source.line}`)
                .then(() => setCopied(true));
            }}
          >
            <code>
        ...
        {sourceLoading && (
          <div className="source-loading">
            <LoaderCircle size={13} /> Reading analyzed source
          </div>
        )}
        {sourceError && (
        ...
        {sourceSnippet && (
          <div
            ...
            aria-label={`Source around line ${sourceSnippet.focusLine}`}
          >
            <div>
              <Code2 size={12} /> ANALYZER CONTEXT
            </div>
            <pre>
              {sourceSnippet.lines.map((line) => (
                <span
                  key={line.number}
                  className={
                    line.number === sourceSnippet.focusLine ? "is-focus" : ""
                  }
                >
                  <i>{line.number}</i>
                  <code>{line.text || " "}</code>
      ...
      <div className="inspector-section">
        <h3>
          <Clock3 size={13} /> RUNTIME STATE
        </h3>
        <div className="runtime-readout">
          <span>Latest event</span>
          <strong>
            {current?.type.replace("span:", "") ?? "not observed"}
          </strong>
        </div>
        {node.meta &&
